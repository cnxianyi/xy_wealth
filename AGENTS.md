...
- Go module 使用占位路径 `github.com/xy-wealth/xy-wealth`；上游仓库地址为 `https://github.com/cnxianyi/xy_wealth`。
...
- Binance 是第一个交易所 provider，按能力区分 Spot 与 USDⓈ-M Futures；后续交易所（如 Bitget）应以独立 provider 扩展，不要把 provider 特有逻辑散落到公共层。
...
- Binance 先接入官方 Spot 基础只读 API，再接入 Futures（USDⓈ-M）基础只读 API；新增接口应同时更新 provider、handler、路由、OpenAPI 规范和测试。
